...
class SearchResult {
  ...
  factory SearchResult.fromMap(Map<String, dynamic> map) => SearchResult(
        ...
        size: map['size'] is int ? map['size'] as int : int.tryParse(_asString(map['size'])),
      );
...
class SearchService {
  ...
  void _init() {
    ...
    _swarmSubscription = VaultSwarm.instance.messageStream.listen(_onSwarmEvent);
  }
  ...
  Future<void> broadcastSearch(String query) async {
    ...
    await VaultSwarm.instance.broadcastMessage(
      'swarm_search',
      {
        'type': 'search_query',
        'payload': {
  ...
  void _onSwarmEvent(Map<String, dynamic> event) {
    ...
    if (type == 'search_query') {
      ...
      TorrentService.instance.handleIncomingSearch(query, requesterId: requesterId);
      return;
    }
    ...
    final idx = _results.indexWhere((r) => r.torrentId == result.torrentId && r.responderId == result.responderId);
    if (idx < 0) {
      _results.add(result);
      _resultsController.add(List.unmodifiable(_results));
    }
